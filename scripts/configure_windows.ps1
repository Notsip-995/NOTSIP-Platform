...
Write-Host 'Everything here is optional except the choices you actually want enabled. Secrets are stored encrypted after this wizard.'
...
Ask 'NOTSIP_VISION_ENABLED' 'Enable visual perception true/false' $false 'true'; Ask 'NOTSIP_PERCEPTION_ENABLED' 'Enable perception pipeline true/false' $false 'true'; Ask 'NOTSIP_PERCEPTION_INTERVAL' 'Perception interval seconds' $false '10'
...
.\.venv\Scripts\python.exe .\scripts\migrate_secrets.py
Write-Host 'Configuration saved. Secrets have been moved into the encrypted local secret store; .env is excluded from Git.' -ForegroundColor Green
