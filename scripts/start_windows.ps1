...
function Get-NotsipConfig {
  $candidates=@(
    ...
    (Join-Path $env:LOCALAPPDATA 'NOTSIP\data\config.json')
  )
...
function Get-NotsipUrl {
  $cfg=Get-NotsipConfig
  $host='127.0.0.1';$port=8765
  if($cfg -and $cfg.settings){
    if($cfg.settings.host){$host=[string]$cfg.settings.host}
    if($cfg.settings.port){$port=[int]$cfg.settings.port}
  }
