...
class MainActivity:Activity(){
 ...
 private fun requestPermissionsIfNeeded(){val req= mutableListOf<String>();if(checkSelfPermission(Manifest.permission.RECORD_AUDIO)!=PackageManager.PERMISSION_GRANTED)req+=Manifest.permission.RECORD_AUDIO;if(checkSelfPermission(Manifest.permission.CAMERA)!=PackageManager.PERMISSION_GRANTED)req+=Manifest.permission.CAMERA;if(checkSelfPermission(Manifest.permission.CALL_PHONE)!=PackageManager.PERMISSION_GRANTED)req+=Manifest.permission.CALL_PHONE;if(checkSelfPermission(Manifest.permission.SEND_SMS)!=PackageManager.PERMISSION_GRANTED)req+=Manifest.permission.SEND_SMS;if(req.isNotEmpty())requestPermissions(req.toTypedArray(),41)}
