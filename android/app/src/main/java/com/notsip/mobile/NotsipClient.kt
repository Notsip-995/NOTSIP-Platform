...
class NotsipClient(private val ctx:Context){
 ...
 private fun req(m:String,path:String,body:String?,requestBody:RequestBody?=null):String{val rb=Request.Builder().url(baseUrl()+path);if(m=="POST")rb.post(requestBody?:body?.toRequestBody("application/json".toMediaType())?:"".toRequestBody());else rb.get();http.newCall(rb.build()).execute().use{if(!it.isSuccessful)throw RuntimeException("HTTP ${it.code}");return it.body!!.string()}}
}
