...
class NotsipClient(private val ctx:Context){
 ...
 fun result(id:String,status:String,result:JSONObject){req("POST","/api/devices/result",JSONObject().put("command_id",id).put("status",status).put("result",result).toString(),null,deviceHeaders())}
 ...
 private fun deviceHeaders()=mapOf("X-NOTSIP-Device-ID" to deviceId(),"X-NOTSIP-Device-Token" to token())
 private fun req(m:String,path:String,body:String?,requestBody:RequestBody?=null,headers:Map<String,String>=emptyMap()):String{val rb=Request.Builder().url(baseUrl()+path);headers.forEach{(k,v)->rb.addHeader(k,v)};if(m=="POST"){val rbod=requestBody?:body?.let{RequestBody.create("application/json".toMediaType(),it)}?:RequestBody.create(null,ByteArray(0));rb.post(rbod)}else rb.get();http.newCall(rb.build()).execute().use{if(!it.isSuccessful)throw RuntimeException("HTTP ${it.code}");return it.body!!.string()}}
}
