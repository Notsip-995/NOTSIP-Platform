...
class NotsipCommandService:Service(){
 ...
 private fun exec(x:JSONObject){val id=x.optString("id");val action=x.optString("action");val p=x.optJSONObject("payload")?:JSONObject();if(id.isBlank())return; if(completed().contains(id)){c.result(id,"SUCCESS",JSONObject().put("action",action).put("verified",true).put("duplicate",true).put("note","command ID already executed on this device"));return};try{
   ...
   markCompleted(id);val result=JSONObject().put("action",action).put("verified",verified);if(note.isNotEmpty())result.put("note",note);c.result(id,status,result)
