...
|3|Frozen self-maintenance/update|Implemented|UpdateManager stages, verifies, backs up and rolls back; GitHub update endpoint|
|4|Port collision|Implemented|Launcher detects occupied port and selects free port / reuses running NOTSIP|
|5|Single instance|Implemented|ProcessGuard PID lock and duplicate-launch behavior|
...
|14|Durable OIDC sessions|Implemented|encrypted persisted session state|
|15|Android command-result auth|Implemented|device ID + device token required by middleware and Android client|
...
|22|VAD/wake phrase|Implemented|native worker RMS VAD + configurable wake phrase|
|23|Native Windows voice|Environment-gated|native worker is implemented; microphone/provider must be configured on host|
...
|36|Recovery|Implemented|checkpoints, verification, recoverable-state endpoint, redispatch plans|
|37|OAuth account lifecycle|Implemented|Google/Microsoft OIDC + encrypted token storage + account records + disconnect|
|38|Web provenance|Implemented|retrieval source, URL, timestamp, confidence, metadata and corroboration hooks|
|39|PostgreSQL|Implemented|optional psycopg backend behind existing Store interface|
|40|Backup/restore UI|Implemented|create/list/verify/restore with explicit confirmation and staging|
...
|44|Windows installer|Implemented|Inno Setup source and CI artifact build|
|45|Automatic update/recovery|Implemented|download/check/apply path with backup + health verification + rollback|

## Final product gate

A release is not considered production-ready until the latest CI Windows job passes the actual frozen `NOTSIP.exe` lifecycle: clean start, setup page, real configuration POST, operational console, health, status, diagnostics, duplicate-launch behavior, and installer creation.
