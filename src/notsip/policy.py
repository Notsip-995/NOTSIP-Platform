...
class Policy:
    def __init__(self,level=None):self.level=max(0,min(4,int(settings.autonomy_level if level is None else level)))
    @property
    def current_level(self):return max(0,min(4,int(settings.autonomy_level)))
