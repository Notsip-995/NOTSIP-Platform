...
async def _safe_validate_id_token(provider,id_token,nonce=''):
    import httpx,jwt
    metadata=provider.metadata or await provider.discover()
    ...
    jwks=response.json();header=jwt.get_unverified_header(id_token);alg=str(header.get('alg') or '');kid=header.get('kid')
...
def attach(app,auth,accounts,settings):
    ...
    auth.oidc.validate_id_token=_safe_validate_id_token.__get__(auth.oidc,type(auth.oidc))
    ...
    async def oauth_callback_hardened(code:str,state:str,notsip_oidc_csrf:str|None=Cookie(default=None)):
        ...
        actor=getattr(__import__('notsip.actor_context',fromlist=['current_actor']),'current_actor')()
        account=accounts.upsert(settings.oidc_provider,subject,claims.get('email',''),auth.oidc.scopes,{'expires_at':tokens.get('expires_at'),'token_type':tokens.get('token_type')},owner=actor);accounts.save_tokens(account['id'],tokens,owner=actor)
