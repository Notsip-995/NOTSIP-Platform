...
import asyncio, json, inspect, hashlib, secrets, socket, os, time, uuid
from contextvars import ContextVar

_current_device: ContextVar[str] = ContextVar('notsip_current_device', default='')


def install(store, jobs, app):
    """Install transactional task/device state transitions on a live Store/Scheduler."""
    original_run_one = jobs.run_one
    original_pull = store.pull_commands
    original_result = store.command_result

    def _sqlite_claim_task(task_id, payload):
        now=time.time()
        with store.lock, store.conn() as c:
            cur=c.execute('UPDATE tasks SET state=?,data=?,updated=?,error=? WHERE id=? AND state=?',('RUNNING',json.dumps(payload),now,'',task_id,'PENDING'))
            if cur.rowcount != 1:
                return None
            row=c.execute('SELECT * FROM tasks WHERE id=?',(task_id,)).fetchone()
            return dict(row) if row else None

    def _postgres_claim_task(task_id, payload):
        now=time.time()
    ...
    def claim_task(task_id, payload):
        return _postgres_claim_task(task_id,payload) if getattr(store,'_backend',None) else _sqlite_claim_task(task_id,payload)
    ...
    async def run_one(task):
        ...
        claimed=claim_task(task['id'],payload)
        ...
        try:
            ...
            if claimed.get('interval_sec'):
                store.task_update(task['id'],state='PENDING',run_at=time.time()+claimed['interval_sec'],data=json.dumps(payload),error='')
            else:
                store.task_update(task['id'],state='COMPLETED',data=json.dumps(payload),error='')
            return {'status':'SUCCESS','result':result,'execution_id':payload['execution_id']}
        except Exception as exc:
            retries=int(claimed.get('retries') or 0)+1; max_retries=int(payload.get('max_retries',jobs.max_retries))
            payload.update({'last_error':str(exc),'failed_at':time.time()})
            if retries<=max_retries:
                backoff=min(900,2**min(retries,9)); store.task_update(task['id'],state='PENDING',run_at=time.time()+backoff,retries=retries,data=json.dumps(payload),error=str(exc))
                return {'status':'RETRYING','error':str(exc),'retry':retries,'backoff':backoff}
            store.task_update(task['id'],state='FAILED',data=json.dumps(payload),error=str(exc),retries=retries)
            return {'status':'FAILURE','error':str(exc),'retries':retries}
    ...
    def atomic_pull(device_id, limit=20):
        ...
        if getattr(store,'_backend',None):
            with store._backend.conn() as c:
                rows=c.execute("SELECT * FROM commands WHERE device_id=%s AND status='PENDING' ORDER BY created LIMIT %s FOR UPDATE SKIP LOCKED",(device_id,limit)).fetchall()
                out=[]
                now=time.time()
                for row in rows:
        ...
        with store.lock, store.conn() as c:
            ...
            for r in rows:
                ...
                if cur.rowcount==1:
                    r['payload']=json.loads(r['payload']); out.append(r)
            c.commit(); return out

    store.pull_commands=atomic_pull

    def atomic_result(command_id,status,result):
        device_id=_current_device.get()
        if not device_id:
    ...
    store.command_result=atomic_result

    @app.middleware('http')
    async def bind_authenticated_device(request, call_next):
        token=None
        if request.url.path=='/api/devices/result':
            ...
            if not device_id or not device_token or not store.device_token_valid(device_id,device_token):
                ...
                return JSONResponse({'detail':'device authentication required'},status_code=401)
            token=_current_device.set(device_id)
        try:
            return await call_next(request)
        finally:
            if token: _current_device.reset(token)
