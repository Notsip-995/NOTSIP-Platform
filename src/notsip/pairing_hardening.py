...
def attach(app,require_auth,store,pairing,auth=None):
    ...
    async def pair_code(_:None=Depends(require_auth)):
        actor=current_actor();code=store.create_pair_code();secret_store.set('pairing:owner:'+code,{'actor':actor,'created_at':time.time(),'expires_at':time.time()+300});return {'code':code,'actor':actor,'expires_in':300}

    @app.post('/api/pair/consume')
    async def pair_consume(body:PairIn,request:Request,_:None=Depends(require_auth)):
        code=body.code.strip().upper();record=secret_store.get('pairing:owner:'+code);actor=current_actor();expires=float(record.get('expires_at',0)) if isinstance(record,dict) else 0
