...
class AccountStore:
    def __init__(self,secrets):self.secrets=secrets;self._lock=threading.RLock()
    ...
    def upsert(self,provider,subject,email='',scopes='',token_meta=None,owner=None):
        ...
        with self._lock:
            d=self._accounts();subject=str(subject or '').strip();key=f'{owner}:{provider}:{subject or uuid.uuid4().hex}';existing=d.get(key,{})
    ...
    def list(self,owner=None):
        owner=self._owner(owner)
        with self._lock:return [x for x in self._accounts().values() if x.get('owner','primary-user')==owner]
    def get(self,account_id,owner=None):
        owner=self._owner(owner)
        with self._lock:
            item=self._accounts().get(account_id);return item if item and item.get('owner','primary-user')==owner else None
    ...
    def tokens(self,account_id,owner=None):
        with self._lock:return self.secrets.get(self._token_key(account_id),{}) if self.get(account_id,owner) else {}
