        ...
        while self.running:
            ...
            try:
                ...
                if self.telemetry and self.health_analytics:
                    try:
                        analysis=self.health_analytics.analyze(120)
                        self.health_analytics.record(self.telemetry())
                        if analysis.get('warnings'):await self.events.publish(Event('health.warning',analysis,'health-analytics'))
                    except Exception as exc:await self.events.publish(Event('telemetry.error',{'error':str(exc)},'health-analytics'))
                ...
                if now>=self.next_proactive:
                    ...
                    if candidates:await self.events.publish(Event('proactive.candidates',{'count':len(candidates),'candidates':candidates[:10]},'intelligence'))
