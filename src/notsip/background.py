...
class BackgroundSupervisor:
    ...
    async def _health_cycle(self):
        ...
        for prediction in (cpu,mem):
            if prediction.get('status')=='SUCCESS' and prediction.get('trend',{}).get('state') in {'DEGRADATION','ANOMALY','PREDICTED_FAILURE'}:
                await self.events.publish(Event('maintenance.prediction',prediction,'predictive-maintenance'))
    async def loop(self):
        while self.running:
            now=time.time()
            try:
                ...
                if now>=self.next_proactive:
                    ...
                    for candidate in candidates[:10]:
                        ...
                        if decision.priority.value>=2:
                            payload=dict(candidate);payload.update({'priority':decision.priority.name,'interrupt':decision.interrupt,'score':decision.score})
                            await self.events.publish(Event('proactive.candidate',payload,'intelligence'))
