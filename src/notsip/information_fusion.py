...
class InformationFusion:
    """Fuse authorized stored evidence and public remote evidence without cross-actor disclosure."""
    ...
    def _authority(row,default_originality=1.0):
        metadata=row.get('metadata') or {};metadata=json.loads(metadata) if isinstance(metadata,str) else metadata
    ...
    def _cluster(self,evidence):
        ...
        for c in clusters:
            ...
            best=max(c,key=lambda x:float(x.get('confidence',0)));originality,corroboration=self._authority(best);independent=len(sources);source_corrob=max(corroboration,independent)
    ...
    def _conflicts(self,evidence):
        ...
        for row in evidence:
            metadata=row.get('metadata') or {};metadata=json.loads(metadata) if isinstance(metadata,str) else metadata;subject=self._norm(metadata.get('subject',''))
        ...
        for subject,rows in by_subject.items():
            ...
            if len(unique)>1:out.append({'subject':subject,'statements':unique,'sources':sorted({r.get('source','unknown') for r in rows}),'status':'CONFLICTING_EVIDENCE','confidence_by_statement':[{'statement':s,'sources':sorted({r.get('source','unknown') for r in rows if self._norm(r.get('statement',''))==self._norm(s)})} for s in unique]})
        return out
    async def fuse(self,query,count=5,actor=None):
        actor=actor or current_actor();query=str(query).strip()
        if not query:raise ValueError('query is required')
        local=self.stored(query,200,actor);remote=[]
        if self.web.enabled:
            remote=await self.web.search(query,max(1,min(int(count),20)))
            for item in remote:self.store.fact(item.get('title',''),'brave',item.get('url',''),.55,{'query':query,'retrieved_at':time.time(),'originality':1.0,'corroboration':0,'actor':actor})
        evidence=[]
        for f in local:
            metadata=f.get('metadata') or {};metadata=json.loads(metadata) if isinstance(metadata,str) else metadata;originality,corroboration=self._authority(f);evidence.append({'kind':'stored_fact','statement':f.get('statement',''),'source':f.get('source',''),'url':f.get('url',''),'confidence':f.get('confidence',0.0),'retrieved':f.get('retrieved'),'metadata':metadata,'originality':originality,'corroboration':corroboration})
