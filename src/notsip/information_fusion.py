...
class InformationFusion:
    ...
    def __init__(self, store, web, similarity_threshold=0.90):
        self.store,self.web=store,web;self.similarity_threshold=float(similarity_threshold)
    @staticmethod
    ...
    def _cluster(self, evidence):
        ...
        for c in clusters:
            ...
            score=min(1.0,0.55*float(best.get('confidence',0))+0.30*min(1,independent/3)+0.15*float(best.get('originality',1)))
            claims.append({'statement':best.get('statement',''),'support':len(c),'independent_sources':independent,'sources':sorted(sources),'confidence':score,'best_source_confidence':float(best.get('confidence',0)),'timestamps':sorted(str(x.get('retrieved','')) for x in c if x.get('retrieved'))})
        return sorted(claims,key=lambda x:x['confidence'],reverse=True)
    def _conflicts(self,evidence):
        by_subject=defaultdict(list)
        for row in evidence:
            subject=self._norm((row.get('metadata') or {}).get('subject',''))
            if subject:by_subject[subject].append(row)
        out=[]
        for subject,rows in by_subject.items():
            unique=[]
            for row in rows:
                if self._norm(row.get('statement','')) not in {self._norm(x) for x in unique}:unique.append(row.get('statement',''))
            if len(unique)>1:out.append({'subject':subject,'statements':unique,'sources':sorted({r.get('source','unknown') for r in rows}),'status':'CONFLICTING_EVIDENCE'})
        return out
    async def fuse(self, query, count=5):
        ...
        if self.web.enabled:
            ...
            for item in remote:self.store.fact(item.get('title',''),'brave',item.get('url',''),.55,{'query':query,'retrieved_at':time.time()})
        evidence=[{'kind':'stored_fact','statement':f.get('statement',''),'source':f.get('source',''),'url':f.get('url',''),'confidence':f.get('confidence',0.0),'retrieved':f.get('retrieved'),'metadata':f.get('metadata') or {}} for f in local]
        evidence += [{'kind':'web','statement':r.get('title',''),'source':'brave','url':r.get('url',''),'confidence':.55,'description':r.get('description',''),'retrieved':time.time(),'metadata':{'query':query}} for r in remote]
