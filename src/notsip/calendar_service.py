...
class CalendarStore:
    ...
    def _parse(self,value):
        ...
        return dt.timestamp()
    def conflicts(self,start,end,exclude=''):
        return [e for e in self.events if e.get('id')!=exclude and e.get('start_ts',0)<end and e.get('end_ts',0)>start]
    def create(self,title,start,end,description='',location='',reminder_minutes=15):
        ...
        conflicts=self.conflicts(st,en)
        event={'id':str(uuid.uuid4()),'title':title,'description':description,'location':location,'start':start,'end':end,'start_ts':st,'end_ts':en,'reminder_minutes':int(reminder_minutes),'created_at':time.time(),'updated_at':time.time()}
        self.events.append(event);self.save();return event,conflicts
    def update(self,eid,**changes):
        item=next((e for e in self.events if e.get('id')==eid),None)
        if not item:raise KeyError(eid)
        start=changes.get('start',item['start']);end=changes.get('end',item['end']);st,en=self._parse(start),self._parse(end)
        if en<=st:raise ValueError('end must be after start')
        conflicts=self.conflicts(st,en,eid);item.update({k:v for k,v in changes.items() if k in {'title','description','location','start','end','reminder_minutes'}});item['start_ts']=st;item['end_ts']=en;item['updated_at']=time.time();self.save();return item,conflicts
    def delete(self,eid):
...
def attach(app,require_auth,root,timezone):
