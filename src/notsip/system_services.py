...
import json, os, platform, shutil, socket, subprocess, tempfile, time, zipfile
...
def _telemetry():
    ...
    try:
        ...
        out['memory'] = {'total': vm.total, 'available': vm.available, 'used': vm.used, 'percent': vm.percent}
        out['cpu_percent'] = psutil.cpu_percent(interval=0.1)
        out['boot_time'] = psutil.boot_time()
        out['net'] = {name: {'is_up': stats.isup, 'speed': stats.speed, 'mtu': stats.mtu} for name, stats in psutil.net_if_stats().items()}
        out['battery'] = None if psutil.sensors_battery() is None else {
            'percent': psutil.sensors_battery().percent,
            'plugged': psutil.sensors_battery().power_plugged,
        }
    except Exception as exc:
        out['metrics_note'] = f'extended psutil metrics unavailable: {exc}'
    return out
...
class WorkflowEngine:
    ...
    async def run(self, steps):
        ...
        for index, step in enumerate(steps):
            ...
            if not objective:
                results.append({'index': index, 'status': 'FAILURE', 'error': 'empty workflow step'})
                break
            result = await self.agent.handle(objective)
            results.append({'index': index, 'objective': objective, 'result': result})
...
def attach(app, require_auth, settings_obj, store, agent, registry):
    workspace = Workspace(Path(settings_obj.data_dir).resolve() / 'workspace')
    workflow = WorkflowEngine(store, agent)
    ...
    register('python_exec', 'Run a time-limited Python program inside the authorized workspace; requires approval.', 'CODE_EXECUTION', Risk.HIGH, {'type':'object','properties':{'code':{'type':'string'},'timeout':{'type':'integer','minimum':1,'maximum':60}},'required':['code']}, lambda code,timeout=30: _python_exec(workspace,code,timeout), True)
    register('simulate', 'Run a deterministic numeric simulation program inside the workspace; requires approval.', 'SIMULATION', Risk.HIGH, {'type':'object','properties':{'code':{'type':'string'},'timeout':{'type':'integer','minimum':1,'maximum':60}},'required':['code']}, lambda code,timeout=30: _python_exec(workspace,code,timeout), True)
...
def _move(workspace, source, target):
    return _rename(workspace, source, target)
...
def _python_exec(workspace, code, timeout=30):
    p=workspace.root/'runtime_exec';p.mkdir(parents=True,exist_ok=True)
    script=p/'run.py';script.write_text(code,encoding='utf-8')
    try:
        r=subprocess.run([os.environ.get('PYTHON','python'),'-I',str(script)],cwd=str(workspace.root),capture_output=True,text=True,timeout=max(1,min(int(timeout),60)))
