...
class PostgreSQLStore:
    ...
    def restore_runtime_state(self,state):
        with self.conn() as c:
            ...
            for e in (state.get('world') or {}).get('entities') or []:c.execute('INSERT INTO entities(id,kind,name,data,updated) VALUES(%s,%s,%s,%s::jsonb,%s)',(e.get('id',''),e.get('kind',''),e.get('name',''),e.get('data','{}') if isinstance(e.get('data','{}'),str) else json.dumps(e.get('data') or {}),float(e.get('updated',time.time()))))
            c.commit()
    ...
    def pull_commands(self,device_id,limit=20):
        with self.conn() as c:
            ...
            if ids:c.execute('UPDATE commands SET status=\'DELIVERED\',updated=%s WHERE id=ANY(%s) AND status=\'PENDING\'',(time.time(),ids));c.commit();return [dict(r) for r in rows]
            c.commit();return []
    def reconcile_commands(self,lease_seconds=None):lease=max(30,int(lease_seconds or os.getenv('NOTSIP_COMMAND_LEASE_SECONDS','300')));cutoff=time.time()-lease
        with self.conn() as c:rows=c.execute("SELECT c.id,c.device_id,c.action,d.status AS device_status,d.last_seen FROM commands c LEFT JOIN devices d ON d.id=c.device_id WHERE c.status='DELIVERED' AND c.updated<%s AND (d.id IS NULL OR d.status='STALE' OR d.status='REVOKED' OR d.last_seen<%s)",(cutoff,cutoff)).fetchall();c.executemany("UPDATE commands SET status='UNKNOWN',updated=%s,result=%s::jsonb WHERE id=%s AND status='DELIVERED'",[(time.time(),json.dumps({'verified':False,'reason':'device did not return a command result before lease expiry'}),r['id']) for r in rows]);c.commit();return [dict(r) for r in rows]
