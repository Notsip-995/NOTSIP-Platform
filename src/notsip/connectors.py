...
class Web:
    ...
    async def search(self,q,count=5):
        ...
        async with httpx.AsyncClient(timeout=20) as c:r=await c.get('https://api.search.brave.com/res/v1/web/search',params={'q':q,'count':count},headers={'Accept':'application/json','X-Subscription-Token':self.key});r.raise_for_status();d=r.json()
...
class Email:
    ...
    @property
    def enabled(self):return bool(self.username and self.password and (self.smtp_host or self.imap_host))
    def send(self,to,subject,body):
        if not self.enabled or not self.smtp_host:raise RuntimeError('SMTP not configured')
        m=EmailMessage();m['From']=self.username;m['To']=to;m['Subject']=subject;m.set_content(body)
        with smtplib.SMTP(self.smtp_host,self.smtp_port,timeout=20) as s:
            if self.smtp_port!=25:s.starttls(context=ssl.create_default_context())
            s.login(self.username,self.password);s.send_message(m)
        return {'status':'SUCCESS','to':to,'subject':subject}
    def search(self,mailbox='INBOX',criteria='ALL',limit=20):
        if not self.enabled or not self.imap_host:raise RuntimeError('IMAP not configured')
        c=imaplib.IMAP4_SSL(self.imap_host);c.login(self.username,self.password);c.select(mailbox,readonly=True);_,d=c.search(None,criteria);ids=d[0].split()[-limit:];out=[]
