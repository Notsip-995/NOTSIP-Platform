...
class Scheduler:
    ...
    def _recover_stale(self):
        ...
        for task in self.store.tasks('RUNNING'):
            ...
            if started and now-started>self.reclaim_after:
                data.update({'recovered_from':data.get('worker_id'),'recovered_at':now});self.store.task_update(task['id'],state='PENDING',run_at=now,data=json.dumps(data),error='reclaimed after worker timeout')
    ...
    def create(self,objective,handler='agent',delay=0,interval=None,data=None,priority=0,idempotency_key='',actor=None):
        ...
        priority=max(0,min(4,int(priority or 0)));payload=dict(data or {});payload.setdefault('actor',str(actor or current_actor()));key=idempotency_key or uuid.uuid4().hex;payload.setdefault('max_retries',self.max_retries);payload.setdefault('idempotency_key',key)
    ...
    async def run_one(self,task):
        ...
        if not fn:
            self.store.task_update(task['id'],state='FAILED',error=f'no handler registered: {handler}');await self._publish('task.failed',{'task_id':task['id'],'objective':task.get('objective',''),'handler':handler,'error':f'no handler registered: {handler}','attempt':int(task.get('retries') or 0)+1});return {'status':'FAILURE','error':'no handler registered'}
