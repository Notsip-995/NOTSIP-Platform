...
def attach(app,require_auth,settings,store):
    ...
    async def setup_readiness(_:None=Depends(require_auth)):
        ...
        checks=[
            ...
            {'id':'email','label':'Email','configured':_configured(settings.email_username) and _configured(settings.email_password) and (_configured(settings.smtp_host) or _configured(settings.imap_host)),'kind':'user_configuration','required':False,'detail':'Configure SMTP/IMAP for mail operations.'},
            ...
            {'id':'voice_hardware','label':'Native voice hardware','configured':platform.system()=='Windows' and bool(getattr(settings,'native_voice_enabled',False)),'kind':'external_environment','required':False,'detail':'Requires Windows and working microphone/audio hardware plus the configured STT/TTS services.'},
        ]
