...
class ContinuousPerception:
    def __init__(self,settings,windows,media,store,events,user_id='primary-user'):
        self.settings=settings;self.windows=windows;self.media=media;self.store=store;self.events=events;self.user_id=user_id;self.last_hash='';self.running=True
    async def once(self):
        ...
        self.store.remember(self.user_id,'perception',obs.get('observation',''),.65,'desktop-vision',{'sha256':digest,'timestamp':time.time()})
