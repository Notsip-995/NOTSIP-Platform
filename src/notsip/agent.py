...
class Agent:
    ...
    def __init__(self,settings,store,policy,registry,provider,world):self.settings=settings;self.store=store;self.policy=policy;self.registry=registry;self.provider=provider;self.world=world;self.user='primary-user'
    def context(self,text):
        now=datetime.now(ZoneInfo(self.settings.local_timezone));return {'time':now.isoformat(),'memory':self.store.memories(self.user,text,15),'conversation':self.store.history(20),'world':self.world.snapshot()}
    async def handle(self,text):
        self.store.message('user',text);m=re.match(r'^remember(?: that)?\s+(.+)$',text,re.I)
        if m:
            v=m.group(1).strip();self.store.remember(self.user,'semantic',v,.95);r=f"I'll remember that: {v}";self.store.message('assistant',r);self.store.audit(self.user,text,'memory','remember','SUCCESS');return {'response':r,'status':'SUCCESS'}
        if not self.provider.enabled:r=self.fallback(text);self.store.message('assistant',r);return {'response':r,'status':'DEGRADED'}
    ...
    def fallback(self,text):
        ...
        return 'NOTSIP is online in degraded mode. Configure a real reasoning model in .env.'
