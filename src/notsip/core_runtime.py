...
reg('read_file','Read a workspace text file.','READ_FILES',Risk.LOW,{'type':'object','properties':{'path':{'type':'string'}},'required':['path']},lambda path:{'status':'SUCCESS','content':WS.read(path)})
reg('write_file','Write a workspace text file.','WRITE_FILES',Risk.MEDIUM,{'type':'object','properties':{'path':{'type':'string'},'content':{'type':'string'}},'required':['path','content']},lambda path,content:{'status':'SUCCESS','path':WS.write(path,content)})
...
reg('self_inspect','Inspect the NOTSIP repository files and SHA-256 manifest.','SELF_MAINTENANCE',Risk.LOW,{'type':'object','properties':{}},lambda:{'status':'SUCCESS','repository':str(ROOT),'files':maint.inventory()})
reg('self_read','Read one source/config file from the NOTSIP repository.','SELF_MAINTENANCE',Risk.LOW,{'type':'object','properties':{'path':{'type':'string'}},'required':['path']},lambda path:{'status':'SUCCESS','path':path,'content':maint.read(path)})
reg('self_verify','Compile and run the NOTSIP regression suite.','SELF_MAINTENANCE',Risk.MEDIUM,{'type':'object','properties':{}},lambda:maint.verify())
reg('self_apply','Apply a Git patch to a temporary self-maintenance branch, test it, and commit only if verification succeeds.','SELF_MAINTENANCE',Risk.HIGH,{'type':'object','properties':{'patch':{'type':'string'},'confirmation':{'type':'string'}},'required':['patch','confirmation']},lambda patch,confirmation:maint.apply_patch(patch,confirmation),True)
...
async def status(_:None=Depends(require_auth)):
    return {'identity':'NOTSIP','version':'0.8.0','autonomy_level':policy.level,'tools':[t.name for t in registry.all()],'devices':store.devices(),'world':world.snapshot(),'tasks':store.tasks(),'capabilities':{'llm':provider.enabled,'fallback_llm':provider.fallback_enabled,'web_search':web.enabled,'email':emailc.enabled,'oauth':oauth.configured,'browser':True,'windows':True,'android_pairing':True,'self_maintenance':True}}
@app.get('/api/memory')
...
async def run_task(task_id:str,_:None=Depends(require_auth)):
    if not store.row('SELECT * FROM tasks WHERE id=?',(task_id,)):raise HTTPException(404,'Task not found')
...
async def oauth_callback(code:str,state:str):
    result=await oauth.exchange(code);safe={k:('[redacted]' if any(x in k.lower() for x in ('token','secret')) else v) for k,v in result.items()};return {'status':'SUCCESS','state':state,'response':safe}
@app.post('/api/events')
...
async def self_apply(body:SelfPatch,_:None=Depends(require_auth)):
    if not policy.decide(Risk.HIGH).allowed or not settings.self_modify_enabled:return {'status':'PARTIAL_SUCCESS','approval_required':True,'error':'self-modification is disabled by policy/config'}
