...
def attach(app):
    ...
    async def config_set_with_session(payload:dict,request:Request):
        ...
        if 'database_url' in settings_payload:
            database_url=str(settings_payload.get('database_url') or '').strip() or 'sqlite:///data/notsip.db'
            settings_payload['database_url']=database_url
            mod.settings.database_url=database_url
