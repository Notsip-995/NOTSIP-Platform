...
class NewsService:
    ...
    @property
    def configured(self):return bool(self.api_key and self.base_url)
...
class RoadNavigationService:
    ...
    async def route(self,origin_lat,origin_lon,dest_lat,dest_lon,profile='driving'):
        ...
        async with httpx.AsyncClient(timeout=30,trust_env=False) as c:r=await c.get(path,params={'overview':'false','steps':'true'});r.raise_for_status();data=r.json()
...
class FlightPlanningService:
    ...
    @property
    def configured(self):return bool(self.provider_url and self.token)
