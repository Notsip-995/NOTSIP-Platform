...
class NodeRegistry:
    """Signed-ish node lease registry using the existing store device table."""
    def __init__(self,store,secret=''): self.store=store; self.secret=secret or ''
    ...
    def register(self,node_id,name,platform,capabilities=None,public_key=''):
        token=secrets.token_urlsafe(32); self.store.pair_device(node_id,name,platform,public_key,token)
        self.store.exec('UPDATE devices SET data=? WHERE id=?',(json.dumps({'capabilities':capabilities or [],'lease_expires':time.time()+90,'registered_at':time.time()}),node_id))
        return {'node_id':node_id,'token':token,'lease_seconds':90}
    def heartbeat(self,node_id,token,capabilities=None,health=None):
        ...
        data=self.store.row('SELECT data FROM devices WHERE id=?',(node_id,)); cur=json.loads(data['data'] or '{}') if data else {}; cur.update({'capabilities':capabilities or cur.get('capabilities',[]),'health':health or {},'lease_expires':time.time()+90,'last_heartbeat':time.time()}); self.store.exec('UPDATE devices SET last_seen=?,status=?,data=? WHERE id=?',(time.time(),'ONLINE',json.dumps(cur),node_id)); return cur
    def reconcile(self):
        now=time.time(); rows=self.store.rows('SELECT id,data,status FROM devices')
        out=[]
        for r in rows:
...
class RecoveryManager:
    ...
    def checkpoint(self,state):
        p=self.dir/(f'checkpoint-{int(time.time())}-{uuid.uuid4().hex[:8]}.json'); p.write_text(json.dumps(state,indent=2,sort_keys=True),encoding='utf-8'); return str(p.relative_to(self.root))
    def latest(self):
        files=sorted(self.dir.glob('checkpoint-*.json')); return json.loads(files[-1].read_text()) if files else None
