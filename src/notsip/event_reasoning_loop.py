...
class EventReasoningLoop:
    ...
    def _event_review(self,event):
        ...
        task=TaskRecord(objective=objective,requester=str(payload.get('actor') or 'system'),context={'trigger_event':event.type,'event_timestamp':event.timestamp,'evidence':payload},priority=int(decision.priority),constraints=['review only','no external action without separate authorization'],required_tools=['make_plan','corroborate'],permissions=[],subtasks=[],state='PENDING',result={},verification={'required':True,'verified':False})
        task_id=self.scheduler.create(objective,handler='agent',delay=0,priority=int(decision.priority),data=task.to_dict(),idempotency_key=f"event-review:{event.type}:{digest}",actor=task.requester)
    ...
    def _task_feedback(self,event):
        ...
        review_id=self.scheduler.create(objective,handler='agent',delay=0,priority=int(task_data.get('priority',0) or 0),data=review_data,idempotency_key=f"task-feedback:{task_id}:{execution_id}:{cycle+1}:{digest}",actor=review.requester)
