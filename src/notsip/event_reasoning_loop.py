...
class EventReasoningLoop:
    ...
    def attach(self):
        ...
        return self
    def _payload_score(self,payload):
        return float(payload.get('importance',.65)),float(payload.get('urgency',.35)),float(payload.get('relevance',.75))
    def handle(self,event):
        ...
        digest=hashlib.sha256(json.dumps(payload,sort_keys=True,default=str).encode()).hexdigest()[:20];objective=f"Review {event.type} and determine whether any authorized follow-up is required. Evidence: {json.dumps(payload,default=str)[:3000]}";task_id=self.scheduler.create(objective,handler='agent',delay=0,priority=int(decision.priority),data={'trigger_event':event.type,'event_timestamp':event.timestamp,'priority':decision.priority.name,'interrupt':decision.interrupt,'review_only':True},idempotency_key=f"event-review:{event.type}:{digest}")
